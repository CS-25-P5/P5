    ...
    for predictions_path, source_name in config_dict.get('MODELS', []):
        if not os.path.exists(predictions_path):
            ...
            try:
                ...
                print(f"Model '{source_name}': {predictions_path} loaded!")
            except Exception as e:
...
#function for running the comparisons
        ...
        try:
            with contextlib.redirect_stdout(tee_stdout):
                ...
                results, filename = run_model_comparison(
                    ...
                    output_prefix=f"li mf rerun, {current_k}_comparison",
                    calculate_ild=CALCULATE_ILD,
                    catalog=CATALOG,
                    #dataset_type="books"
                    dataset_type = "movies"

...
model = NNforBPR(number_users=numberofusers, number_items=numberofitems, emb_dim=64, hidden_layers=[64])
...
def validate_bpr():
    ...
    prediction_val_dataset.to_csv("data/Predictions_val_5M_goodbooks(MLPwithBPR)/BPRnn_OneLayer_embed64_lr0001_optimizeradam_5M.csv", index = False)
    
    #Add 2-3 lines about time and GPU usage:
    with open("data/Predictions_val_5M_goodbooks(MLPwithBPR)/BPRnn_OneLayer_embed64_lr0001_optimizeradam_5M.csv", "a") as file:
        file.write("\n")
...
def test_bpr(model, testdataloader, device):
    ...
    prediction_test_dataset.to_csv("data/Predictions_test_5M_goodbooks(MLPwithBPR)/BPRnn_OneLayer_embed64_lr0001_optimizeradam_5M.csv", index = False)

    with open("data/Predictions_test_5M_goodbooks(MLPwithBPR)/BPRnn_OneLayer_embed64_lr0001_optimizeradam_5M.csv", "a") as file:
        file.write("\n")

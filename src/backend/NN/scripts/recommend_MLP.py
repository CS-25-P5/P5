...
from groundtruth_test_allitems import split_train_val_test

device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
print(f"Using device: {device}")

df = pd.read_csv("src/backend/Datasets/ratings_100K_movies.csv")
...
def preds_to_csv(model, test_set, csv_fn):
    user_preds = []
    test_user_ids = df['userId'].unique()
    n_movies = df['movieId'].nunique()

    for user in test_user_ids:
        user_id, movie_ids, movie_scores = get_topK(model, user, df, n_movies, False)
...
def grid_search(h_params, file_paths, epochs, patience):
    ...
    for params in all_combs:
        ...
        # Save predictions
        preds_to_csv(model, df, f"{file_paths[1]}_{len(params['layers'])}layers_embed{params['embed_len']}_lr{params['lr']}_batch{params['batch_size']}.csv")
...
output_path = "src/backend/Datasets/train_val_test/ml100k/"
x_train, x_valid, x_test = split_train_val_test(df, "userId", "movieId", output_path, output_path, output_path, "ml100k")
...
epochs = 500
patience = 5
file_paths = ["src/backend/results/MLP_2/ml100k/loss_results/MLP", "src/backend/results/MLP_2/ml100k/predictions/MLP"]
...
test_gt.to_csv("src/backend/results/MLP_2/ml100k/predictions/ground_truth", index=False)

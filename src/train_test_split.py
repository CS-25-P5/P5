...
def generate_movies_file(movies_df, links_df):
    """
    Creates a movies file aligned with MovieLens ratings,
    using TMDB itemId as the unified item identifier.
    """
    ...
    # Clean movies file
    if "itemId" not in movies.columns:
        raise ValueError("movies_df must contain 'itemId' column (TMDB ID).")

    movies["itemId"] = movies["itemId"].astype(str)

    # Merge links and movies on TMDB ID
    merged = links.merge(
        movies,
        left_on="tmdbId",
        right_on="itemId",
    ...
    movies_final = merged[["movieId", "itemId", "title", "genres"]].copy()

    # Rename movieId → the ML ID which ratings file uses
    movies_final.rename(columns={"movieId": "ml_movieId"}, inplace=True)

    # Deduplicate
    movies_final.drop_duplicates(subset="itemId", inplace=True)

    print(f"Movies file created with {len(movies_final):,} unique movies.")
...
def split_ratings(
    ...
    for user_id, user_ratings in filtered_df.groupby('userId'):
        ...
        test = user_ratings.iloc[:n_test].reset_index(drop=True)
        val = user_ratings.iloc[n_test:n_test + n_val].reset_index(drop=True)
        train = user_ratings.iloc[n_test + n_val:].reset_index(drop=True)
...
# links_df = pd.read_csv(os.path.join(base_dir, "datasets/MovieLens", "links.csv"))
# moives_df = generate_movies_file(movies_df, links_df)
# movies_df.to_csv(os.path.join(output_dir_rating, "movies.csv"), index=False)
...
input_books_csv = os.path.join(base_dir, "datasets/GoodBooks", "movies_metadata.csv")

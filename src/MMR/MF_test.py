...
output_dir = os.path.join(base_dir,"../datasets/mmr_data", DATASET_NAME)




def run_mf_pipeline(
    ratings_path, movies_path, output_dir=None, top_n=10, 
    chunksize_movies=10000,k=20, alpha=0.01, lambda_=0.1, n_epochs=50
):
  ...
  movie_user_rating, genre_map, all_genres = load_and_prepare_matrix(ratings_path, movies_path, nrows_movies=chunksize_movies)
  ...
  save_path = os.path.join(output_dir, "mf_test_predtions.csv")
